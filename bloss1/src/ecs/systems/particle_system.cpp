...
namespace bls
{
    ...
    Emitter::Emitter(const vec3& center, bool particle_2D)
    {
        ...
        particle_shader = Shader::create("particle", "bloss1/assets/shaders/particles/particle.vs", "bloss1/assets/shaders/particles/particle_texture.fs");
        quad = std::make_unique<Quad>(renderer);
        pool_index = 999;
        particle_pool.resize(pool_index + 1);
        particle_texture = Texture::create("particle", "bloss1/assets/textures/particles/particle_black.png", TextureType::Diffuse);
